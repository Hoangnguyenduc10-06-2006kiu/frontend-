...
function render() {
  ...
  let messButtonList = `<button onclick="newTabs(${paqe - 1})" ${
    paqe === 1 ? "disabled" : ""
  }>Previous</button>`;

  for (let i = 1; i <= totalPaqe; i++) {
    messButtonList += `<button onclick="newTabs(${i})" ${
      paqe === i ? "disabled" : ""
    }>${i}</button>`;
  }

  messButtonList += `<button onclick="newTabs(${paqe + 1})" ${
    paqe === totalPaqe ? "disabled" : ""
  }>Next</button>`;
